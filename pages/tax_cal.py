...
st.markdown("### 1. 자산 취득 및 폐업 연월 입력")
...
with col1:
    acq_year = st.selectbox("취득 연도", years, index=years.index(2023) if 2023 in years else 0)
with col2:
    acq_month = st.selectbox("취득 월", months, index=0, format_func=lambda x: f"{x}월")

with col3:
    close_year = st.selectbox("폐업 연도", years, index=0)
with col4:
    close_month = st.selectbox("폐업 월", months, index=datetime.date.today().month - 1, format_func=lambda x: f"{x}월")
...
if st.button("세금 계산하기", type="primary", use_container_width=True):
    if acq_year > close_year or (acq_year == close_year and acq_month > close_month):
        st.error("취득 연월이 폐업 연월보다 늦을 수 없습니다.")
    else:
        # 과세기간 계산 로직
        y1, m1 = acq_year, acq_month
        y2, m2 = close_year, close_month
